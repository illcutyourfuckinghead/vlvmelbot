...
async def message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Настроение":
        ...
        await update.message.reply_text("что снилось?")

    elif text == "Таблетки":
        await update.message.reply_text("ты выпила таблетки?")

    elif text == "Музыка":
        await update.message.reply_text("что сегодня?")

    elif text == "Поговорить":
        await update.message.reply_text("что такое пупсеночек?")
